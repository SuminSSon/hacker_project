...
function ReadPost (props) {
    ...
    useEffect(() => {
        const getBoardSubjectCommentList = async () => {
            try {
                const callUrl = serverUrl + 'board/subject/comment/list?boardNumber=' + post.board_number;
                ...
                for (const comment of getBoardInfoCommentListJson) {
                    const date = comment.comments_date.split('T')[0] + ' ' + comment.comments_date.split('T')[1].split('.')[0];
                    const temp = {
    ...
    function PostInfo() {
        return(
            <View style={styles.postInfoWrap}>
                ...
                <View style={styles.postProfNameWrap}>
                    <Text style={{fontSize: 20 ,color:'#3E3F42'}}>{post.board_prof}</Text>
                </View>
            </View>
        );
    };
    ...
    function PostContent() {
        return(
            <View style={styles.postContentWrap}>
                <Text style={{fontSize: 20, marginVertical: 20}}>{post.board_title}</Text>
        ...
        return(
            <View style={styles.writeCommentWrap}>
                ...
                <TouchableOpacity style={styles.writeCommentButton}
                    ...
                    <Text style={{color:'#FFFFFF'}}>댓글</Text>
                </TouchableOpacity>
            </View>
        );
    };
    ...
    const writeComment = async () => {
        await postBoardSubjectCommentWrite();
        setApiCallDone(false);
        forceUpdate();
    };

    const postBoardSubjectCommentWrite = async () => {
        try {
            const callUrl = serverUrl + 'board/subject/comment/write';
            console.log(callUrl);
